...
import {
  HEAD, BODY, C_INK, C_INK_SOFT, C_MUTED, C_GOLD, C_DAWN, C_RULE, C_REVEAL_GRAD,
...
function Snippet() {
  ...
  if (!sign) return null;
  const canSpinAgain = spinNumber < FREE_SPINS_BEFORE_FORM;
  const copy = STATIC_COPY.screen3;

  return (
    <main
      className="relative min-h-screen overflow-hidden"
      style={{ background: C_REVEAL_GRAD, color: C_INK_SOFT }}
    >
      <StarField density={120} opacity={0.7} seedOffset={777} />
      <div className="relative">
        ...
        `}</style>
        <section className="relative mx-auto flex max-w-2xl flex-col items-center px-[clamp(1rem,5vw,3rem)] pt-[clamp(2.5rem,6vh,4.5rem)] pb-[clamp(3rem,6vh,5rem)] text-center">
          <div
            className="w-full"
            style={{
              background:
                "linear-gradient(180deg, rgba(40, 24, 56, 0.55) 0%, rgba(20, 12, 30, 0.55) 100%)",
          ...
          {/* Spin again — only on snippets 1 and 2 */}
          {canSpinAgain && (
